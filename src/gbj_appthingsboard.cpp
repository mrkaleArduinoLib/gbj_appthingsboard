...
{
  if (callbacks_.size() == 0 || !isConnected() || isSubscribed())
  {
    return setLastResult();
  }
  // All consequent data processing will happen in callbacks
  if (thingsboard_->RPC_Subscribe(callbacks_.data(), callbacks_.size()))
  {
